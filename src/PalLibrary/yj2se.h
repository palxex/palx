...
bool Pal::Tools::EncodeYJ2StreamInput(void* pvState, const void* Source, uint32 SourceLength, bool bFinished)
{
	...
	if (state && state->bInitialed && !state->bFinished)
	{
		...
		if (Source == NULL || SourceLength == 0)
			return true;
		if ((end = state->uiIEnd + SourceLength) >= state->uiILength)
		{
			uint8* pbuf;
			uint32 len = end / state->uiIGrowBy + state->uiIGrowBy;
...
bool Pal::Tools::EncodeYJ2StreamOutput(void* pvState, void* Destination, uint32& Length, uint32& Bits)
{
	...
	if (state && state->bInitialed)
	{
		...
		if (!state->bOutput)
		{
			if (Length < 4)
				return false;
			*((uint32*)Destination) = state->length;
			dest = (uint8*)Destination + 4;
			destlen -= 4; addlen = 4;
			state->length = 0;
			state->bOutput = true;
		}
		...
		if (!EncodeYJ2StreamProcess(state, dest, dptr, destlen << 3))
			return false;
		if (state->uiIStart > 0x1000 && state->uiIEnd >= 0x1002 && state->uiIStart < state->uiIEnd && state->top > 0x1000)
		{
